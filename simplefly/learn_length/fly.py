...
class Firefly():
    ...
    def init_pattern(self):
        l = r.randint(MIN_LENGTH,MAX_LENGTH)
        p = [0] * l
    ...
    def mutate(self):
        l = len(self.pattern)
        m_choice = [0,1,2,3,4]
        if sum(self.pattern) == l: #full flash
            m_choice.remove(0)
            m_choice.remove(2)
        if sum(self.pattern) == 1: #only one flash
            m_choice.remove(1)    
        if l == MAX_LENGTH: #max length
            m_choice.remove(3)
        elif l == MIN_LENGTH: #min length 
            m_choice.remove(4)
