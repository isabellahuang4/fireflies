#try to generalize to multiple species
import sys
import random as r
...
def main(args):
    ...
    for epoch in range(EPOCHS):
        ...
        for f in range(floor((NUM_SPECIES * NUM_EACH)/2)):
            i = 2*f
            j = i+1
            #same species
            if fireflies[i].same_species(fireflies[j]):
                #both no pattern
    ...
    print(fireflies[0].pattern)
    print(fireflies[NUM_EACH].pattern)
    print(fireflies[(NUM_EACH*2)].pattern)
